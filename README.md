...
也能操控角色：移动、确认、取消、翻页、冲刺、Ctrl 文本快进，或者一次提交一整套连续动作，比如"向上跑三步，再打开菜单看装备"。`dash_up/down/left/right` 会在同一帧组合方向与游戏的 Shift/冲刺键；`text_skip` 对应 Ctrl，可用 `repeat` 连续保持。
...
| 高层动作 | `black_souls_navigate` · `black_souls_interact` · `black_souls_battle_options` · `black_souls_battle_action` · `black_souls_advance_dialogue` · `black_souls_save` · `black_souls_load` | 自动寻路、走到事件旁交互、列出本回合全部可选项（含特技/魔法二级菜单每一条的消耗与是否可用）、一次调用完成战斗回合（攻击/特技/魔法/防御/道具/逃跑）、推进对话并选择选项、存档落盘校验与读档；菜单导航全部闭环回读真实光标，已通过真实游戏校准 |
...
{
  "steps": [
    { "action": "dash_up", "repeat": 3 },
    { "action": "text_skip", "repeat": 30 }
  ]
}
```

仅对独立 MCP 游戏副本启用可选的引擎快进：在游戏目录下创建
`BridgeRuntime/frame_rate.txt`，内容写一个 `30`–`120` 的整数。有效配置会跨 MCP 游戏重启保留；删除此文件即恢复游戏启动时帧率；
`black_souls_get_state` 的 `frame_rate` 字段返回实际值。快进只缩短真实游戏帧等待，不直接改魂、
属性、掉落、敌人或剧情。调高前应先保存并用少量战斗验证先手、伤害与结算；高难度仍可能导致死亡和掉魂。
